...
- Task 16 framework hardening: device request expiry now uses the live dispatch clock, and idempotent replay is scoped by account and capability and rejects changed request signatures. The complete Swift suite passes 18 XCTest tests plus 31 Swift Testing tests.
- Task 16 JianHao integration: the four canonical training capabilities now execute through a device bridge into the existing local CapabilityKernel and stores. The bridge translates only protocol naming differences, rejects identity/source injection, preserves thread/run ownership, requires device confirmation, and verifies idempotent local read-back. Asia/Shanghai is the stable training calendar; the end-to-end shoulder plan writes tomorrow while leaving today unchanged. Six focused Simulator suites pass, the bridge suite passes after a clean simulator reboot, and the gateway suite passes 283 tests.
- Task 17 JianHao integration: training preparation now patches one stable `training.plan.draft@1` surface through local context, equipment/limitations, candidate search, and editable draft stages. Generic Agent text/activity uses BeaconAgentKit SwiftUI components; the product renderer keeps replace, set adjustment, reorder, date, confirmation, and committing-state behavior. The complete iPhone 17 / iOS 26.5 Simulator suite passes 1,693 tests. A real local "安排一下明天练肩" run exposed all four activity rows and the editable tomorrow card; light and dark screenshots were captured in the JianHao evidence directory.
- Task 18 framework implementation: a closed JSON Schema and matching Python/Swift contracts now define source provenance, commercial-reuse status, locale/version, retrieval adapter, chunk schema, citation policy, safety boundaries, review expiry, provider-neutral corpus/query/passage/result types, and retriever protocols. Evidence-marked claims fail closed unless every referenced passage was retrieved and cited with a matching source. The complete Python suite passes 72 tests and Swift passes 18 XCTest tests plus 34 Swift Testing tests.
